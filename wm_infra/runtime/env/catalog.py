"""Environment catalog — thin adapter over EnvRegistry for backward compatibility.

The old hard-coded ``LearnedEnvCatalog`` is preserved as an API surface so
existing call sites (manager.py, tests) keep working.  Internally it delegates
to :class:`EnvRegistry` and :func:`register_defaults`.
"""
...
class LearnedEnvCatalog:
    ...
    def __init__(self, temporal_store: TemporalStore, *, device: torch.device, dtype: torch.dtype) -> None:
        ...
        self.device = device
        self.dtype = dtype
        self._registry = EnvRegistry()
        register_defaults(self._registry, device=device, dtype=dtype)
        # Backward-compat attributes that tests access directly
        self.world_model = self._registry.resolve("toy-line-v0").world_model
        genie_entry = self._registry.resolve("genie-token-grid-v0")
        self.genie_world_model = genie_entry.metadata.get("genie_world_model", genie_entry.world_model)

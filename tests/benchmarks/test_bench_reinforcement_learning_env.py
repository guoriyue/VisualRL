...
def test_run_benchmark_emits_required_reinforcement_learning_metrics(tmp_path):
    ...
    assert summary["schedule_latency_ms"]["mean"] >= 0.0
    assert summary["transition_latency_ms"]["mean"] >= 0.0
    assert summary["reward_latency_ms"]["mean"] >= 0.0
    assert summary["persist_latency_ms"]["mean"] >= 0.0
    assert summary["chunk_count"]["mean"] >= 1.0
    assert summary["max_chunk_size"]["max"] >= 1.0
    assert summary["state_bytes"]["mean"] > 0.0
    assert summary["state_count"]["mean"] >= 1.0
    assert summary["queue_wait_ms"]["mean"] >= 0.0
